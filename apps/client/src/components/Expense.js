...
export default function Expense({ expense }) {
    ...
    return (
        <div key={expense.id} className="relative h-40 w-48 p-4 flex flex-col justify-evenly items-start rounded-md ring-1 ring-neutral-300">
            ...
            <p>{(new Date(expense.timestamp).toLocaleString())}  </p>
        </div>
    )
}