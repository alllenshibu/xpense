...
const ExpenseExplorer = ({ user }) => {
  ...
  return (
    <div className="h-full flex flex-row justify-center items-center gap-4 flex-wrap">
      {expenses.map((items) => {
        return (
          <div className="bg-black h-64 w-48 p-4 flex flex-col justify-evenly items-center rounded" key={items.exp_id}>
            <div className="text-xl">{items.name}</div>
            <div className="text-2xl font-bold">Rs. {items.owe_amount}</div>
            <div className="">{items.c_name}</div>
            <div className="text-xs">{new Date(items.date).toLocaleString().toString()}</div>
          </div>
        );
      })}
    </div>
