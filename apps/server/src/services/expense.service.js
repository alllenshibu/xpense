...
const addNewExpense = async (user, expense) => {
  ...
  const exp_id = await pool.query(
    "INSERT INTO expenses (payer_id, amount, name, date) VALUES ($1, $2, $3, $4, $5) RETURNING exp_id;",
    [user_id, expense.amount, expense.name, expense.date ]
  ).then((resp) => {
 ...
 try{   expense.group.map(async (share)=>{
...
const getExpenseByCategories = async (user_id) => { 
      const res = await pool.query(
        "SELECT SUM(shares.owe_amount), categories.c_name FROM shares JOIN categories ON shares.sh_cid = categories.c_id WHERE shares.fr_id = $1 GROUP BY categories.c_name;", [user_id]) 
      .then((res) => {
        return res.rows
      } 
      )
...
const getCategoryExpenses = async (user_id , category_id) => {
  const res = await pool.query(
    "SELECT * FROM shares JOIN expenses ON shares.sh_expid = expenses.exp_id JOIN categories ON shares.sh_cid = categories.c_id WHERE shares.fr_id = $1 AND shares.sh_cid = $2;", [user_id , category_id]).then
...
module.exports = { getExpenseDetails, getAllExpenses, getExpenseByCategories, addNewExpense , addShare , getCategoryExpenses}
